...
real and generated images. The Wasserstein metric instead looks at the distribution 
of each variable (i.e., each color of each pixel) in the real and generated images, 
and determines how far apart the distributions are for real and generated data. 
The Wasserstein metric looks at how much effort, in terms of mass times distance, 
it would take to push the generated distribution into the shape of the real distribution,
 hence the alternate name “earth mover distance.” Because the Wasserstein metric 
 is no longer evaluating whether an image is real or not, but is instead providing 
 criticism of how far the generated images are from the real images, the 
 “discriminator” network is referred to as the “critic” network in the Wasserstein 
 architecture
 ref: https://www.toptal.com/machine-learning/generative-adversarial-networks
"""
# General libraries
import pickle
import pandas as pd
...
def generator_network(x, data_dim, base_n_count):
    ...
    base_n_count : Integer
        Number of neurons for the hidden layers.

    Returns
    -------
    x : Keras Network
        Final generator network model
    """
...
def discriminator_network(x, data_dim, base_n_count):
    ...
    -------
    x : Keras Network
        Final discriminator network model
    """
    x = layers.Dense(base_n_count*4, activation='relu')(x)
...
def critic_network(x, data_dim, base_n_count):
    """
    Critic network definition for WGAN
...
def define_models_GAN(rand_dim, data_dim, base_n_count, type=None):
    """
    Function to define the Keras Network models

    Parameters
    ----------
    rand_dim : Integer
        Dimension of random input noise.
    data_dim : Integer
        Number of features of input data.
    base_n_count : Integer
        Number of neurons for the hidden layers.
    type : String
        Whether to use Wasserstein distance (EM distance or not). 
        The default is None.

    Returns
    -------
    generator_model : Keras Network
        Generator Model.
    discriminator_model : Keras Network
        Generator Model.
    combined_model : Keras Network
        Generator Model.
...
def em_loss(y_coefficients, y_pred):
    """
    Earth Mover distance (Wasserstein Loss). Weighted average of the critic 
    network output. Is defined separately so it can be fed as a loss function 
    to the optimizer in the WGANs.
...
def training_steps_WGAN(model_components):
    ...
    for i in range(starting_step, starting_step+nb_steps):
        ...
        if not i % 10: # 2x faster than testing each step...
            ...
            test_size = 100 # test using all of the actual fraud data, hardcoded to 492
...
def adversarial_training_GAN(arguments, train, data_cols, label_cols=[], 
    ...
    data_cols : TYPE
        DESCRIPTION.
    label_cols : TYPE, optional
        DESCRIPTION. The default is [].
    seed : TYPE, optional
        DESCRIPTION. The default is 0.
    starting_step : TYPE, optional
        DESCRIPTION. The default is 0.
