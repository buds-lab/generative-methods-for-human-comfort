...
class TableGan(object):
    ...
    def build_model(self):
        ...
        if self.y_dim:
            self.c_loss = tf.reduce_mean(
                input_tensor=sigmoid_cross_entropy_with_logits(self.C_logits, y_normal))
            self.g_loss_c = tf.reduce_mean(
                input_tensor=sigmoid_cross_entropy_with_logits(self.C_logits_, y_normal))
        ...
        self.D_features_mean = tf.reduce_mean(input_tensor=self.D_features, axis=0, keepdims=True)
        self.D_features_mean_ = tf.reduce_mean(input_tensor=self.D_features_, axis=0, keepdims=True)

        self.D_features_var = tf.reduce_mean(input_tensor=tf.square(self.D_features - self.D_features_mean), axis=0, keepdims=True)

        self.D_features_var_ = tf.reduce_mean(input_tensor=tf.square(self.D_features_ - self.D_features_mean_), axis=0,
                                              keepdims=True)
        ...
        self.info_loss = tf.add(tf.maximum(x=0.0, y=tf.reduce_sum(input_tensor=tf.abs(self.gmean - self.gmean_) - self.delta_mean))
                                , tf.maximum(x=0.0, y=tf.reduce_sum(input_tensor=tf.abs(self.gvar - self.gvar_) - self.delta_var)))
    ...
    def train(self, config): # original: experiment as parameter
